...
export class FreeProviderRouter {
  ...
  /**
   * Generate Image via OpenRouter or Gemini (if available)
   */
  async generateImage(params) {
    const { prompt, model, provider = 'openrouter' } = params;
    const apiKey = this.apiKeys[provider];

    if (!apiKey) throw new Error(`API key not configured for ${provider}`);

    console.log(`🎨 Generando imagen con ${provider} (modelo: ${model || 'default'})...`);
    ...
    if (provider === 'openrouter') {
      ...
      const response = await axios.post(this.providers.openrouter.endpoint, {
        model: selectedModel,
        prompt: prompt, // Para modelos SDXL/Flux
        messages: [{ role: 'user', content: prompt }], // Para DALL-E 3 vía OpenRouter
      ...
      // OpenRouter devuelve la imagen en data[0].b64_json o url
      const imageData = response.data.data?.[0]?.b64_json || response.data.data?.[0]?.url;
      if (!imageData) throw new Error('No se recibió imagen del proveedor');
      ...
      return {
        ...
        model: selectedModel,
        data: imageData,
        isBase64: !!response.data.data?.[0]?.b64_json
      };
    }

    throw new Error('Image generation not implemented for this provider yet');
  }
