...
const AURORA_PRESENCE = {
  ...
  systemPrompt: `Eres Aurora AI, asistente de desarrollo para TradeShare.
- SIEMPRE responde en ESPAÑOL除非用户明确要求其他语言
- Sé directo, conciso y técnico
- Prioriza seguridad y mejores prácticas
...
async function startDaemon() {
  ...
  console.log('🔌 Inicializando presencia de Aurora en el chat...\n');
  console.log('   Proveedores: groq, kimi, openrouter');
  console.log('   Modelos: llama-3.3-70b, kimi-k2-instruct, qwen-2.5-coder');
  console.log('   Idioma: ESPAÑOL (predeterminado)');
  console.log('   Características: code-review, memory-leak-detection, performance-monitoring\n');

  console.log('✨ ¡Aurora ahora está siempre presente en tu chat!\n');
  console.log('   Comandos disponibles:');
  console.log('   - @aurora help          → Mostrar comandos disponibles');
  console.log('   - @aurora review [file] → Code review');
  console.log('   - @aurora analyze       → Análisis profundo');
  console.log('   - @aurora optimize      → Sugerencias de rendimiento');
  console.log('   - @aurora memory        → Verificación de memoria');
  console.log('   - @aurora status        → Estado del sistema\n');
}
