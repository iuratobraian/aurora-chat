...
async function chatReply(query) {
  ...
  // Fallback: Aurora Brain (Ollama)
  const brain = await askOllama(query);
  if (brain.ok) {
    return `${paint(COLOR.magenta, "🧠 Aurora Brain:")} ${brain.answer}`;
  }

  return "Consulta no resuelta en la base local y el Cerebro (Ollama) no está disponible. Intentá con /help, /status, /tasks, /knowledge <query>, /web <query> o /create <kind> <name>.";
}
...
async function handleInicioProtocol() {
  console.log(paint(COLOR.bold + COLOR.cyan, "\n🚀 ACTIVANDO PROTOCOLO DE INICIO (AMM)..."));
  console.log(paint(COLOR.gray, "1. Sincronizando lectura de documentos mandatorios..."));
  ...
  const files = [
    ...
    ".agent/skills/agents/PROTOCOL.md",
  ...
  console.log(paint(COLOR.gray, "\n2. Ejecutando npm run inicio..."));
  await runAllowedConsoleCommand("npm run inicio");

  console.log(paint(COLOR.green, "\n✅ Protocolo de inicio completado. Sistema listo."));
}
...
function printHeader() {
  ...
  panel("Session", [
    ...
    `codex: ${local.codex.error ? paint(COLOR.red, "error") : (local.codex.instalado ? paint(COLOR.green, "ready") : paint(COLOR.yellow, "missing"))}`,
    `opencode: ${local.opencode.error ? paint(COLOR.red, "error") : (local.opencode.instalado ? paint(COLOR.green, "ready") : paint(COLOR.yellow, "missing"))}`,
    `qwen: ${local.qwen.error ? paint(COLOR.red, "error") : (local.qwen.instalado ? paint(COLOR.green, "ready") : paint(COLOR.yellow, "missing"))}`
  ]);
...
function runAllowedConsoleCommand(input) {
  ...
  return new Promise((resolve) => {
    const child = spawn("powershell.exe", ["-NoLogo", "-NoProfile", "-Command", trimmed], {
      cwd: active.ruta,
      stdio: "inherit",
      shell: true
    });
...
async function handleCommand(input) {
  ...
  if (!trimmed.startsWith("/")) {
    const isInicio = /^(ejecuta|lanza|inicia|activar|activando)?\s*(el\s+protocolo\s+)?inicio$/i.test(trimmed);
    if (isInicio) {
      await handleInicioProtocol();
    } else if (looksLikeCodingTask(trimmed)) {
