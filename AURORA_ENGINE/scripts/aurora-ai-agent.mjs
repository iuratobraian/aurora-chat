...
import { loadAuroraEnv as loadENV } from './load-aurora-env.mjs';
import { aiThrottler, providerSelector } from './aurora-ai-utils.mjs';
...
const TASK_PATTERNS = {
  codeReview: {
    ...
    provider: 'kimi',
    alternatives: ['groq', 'deepseek', 'openrouter'],
    model: 'quality',
  ...
  explanation: {
    ...
    model: 'quality',
    reason: '💎 Explicación detallada (GRATIS)',
    subagents: []
  },
  learning: {
    keywords: ['enséñame', 'tutorial', 'guía', 'aprende', 'curso'],
    provider: 'glm',
    alternatives: ['kimi', 'groq'],
    model: 'quality',
    reason: '📚 Explicación didáctica (GRATIS)',
    subagents: ['tutor']
  },
  quick: {
...
function classifyTask(prompt) {
  ...
  const availability = Object.fromEntries(
    Object.entries(PROVIDERS).map(([key, p]) => [key, !!p.apiKey || key === 'ollama'])
  );

  for (const [type, pattern] of Object.entries(TASK_PATTERNS)) {
    if (pattern.keywords.some(kw => lowerPrompt.includes(kw))) {
      const selectedProvider = providerSelector.select(
        pattern.provider, 
        pattern.alternatives, 
        availability,
        0.3 // 30% de probabilidad de usar alternativo
      );
      ...
      return {
        ...
        model: pattern.model,
        reason: `${pattern.reason} (vía ${selectedProvider})`,
        subagents: pattern.subagents
      };
    }
  }
  
  // Default: Groq con alternativas
  const defaultProvider = providerSelector.select(
    'groq', 
    ['openrouter', 'kimi'], 
    availability,
    0.2
  ...
  return {
    ...
    model: 'fast',
    reason: `⚡ Tarea general (vía ${defaultProvider})`
  };
}
...
- Prioriza legibilidad`;
  ...
  async chat(prompt, options = {}) {
    ...
    console.log(`[DEBUG] providerName: ${providerName}`);
    ...
    try {
      ...
      switch (providerName) {
        case 'groq':
          result = await aiThrottler.enqueue('groq', () => callGroq(prompt, modelType, systemPrompt));
          break;
        case 'kimi':
          result = await aiThrottler.enqueue('kimi', () => callKimi(prompt, modelType, systemPrompt));
          break;
        case 'glm':
          result = await aiThrottler.enqueue('glm', () => callGLM(prompt, systemPrompt));
          break;
        case 'claude':
          result = await aiThrottler.enqueue('claude', () => callClaude(prompt, systemPrompt));
          break;
        case 'deepseek':
          result = await aiThrottler.enqueue('deepseek', () => callDeepSeek(prompt, systemPrompt));
          break;
        case 'openrouter':
          result = await aiThrottler.enqueue('openrouter', () => callOpenRouter(prompt, modelType, systemPrompt));
          break;
        case 'ollama':
          result = await callOllama(prompt, systemPrompt); // Ollama local no necesita throttling
          break;
        default:
      ...
      for (let i = currentIndex + 1; i < fallbackOrder.length; i++) {
        ...
        try {
          ...
          switch (fallbackProvider) {
            case 'groq':
              fallbackResult = await aiThrottler.enqueue('groq', () => callGroq(prompt, 'quality', systemPrompt));
              break;
            case 'kimi':
              fallbackResult = await aiThrottler.enqueue('kimi', () => callKimi(prompt, 'quality', systemPrompt));
              break;
            case 'openrouter':
              fallbackResult = await aiThrottler.enqueue('openrouter', () => callOpenRouter(prompt, 'free', systemPrompt));
              break;
            case 'ollama':
...
if (process.argv[1] && path.resolve(process.argv[1]) === SCRIPT_PATH) {
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`
  🧠 AURORA AI AGENT - TradeShare 2025 Multi-Provider Orchestrator

  Uso:
    node aurora-ai-agent.mjs                    # Modo interactivo
    node aurora-ai-agent.mjs "pregunta"         # Consulta directa
    node aurora-ai-agent.mjs --status           # Ver estado de providers
    node aurora-ai-agent.mjs --model groq "..." # Forzar un proveedor

  Providers configurados:
    - Groq (Llama 3.3 70B)     ⚡ Ultra-rápido (0.7s)
