...
async function callGroq(prompt, modelType = 'quality', systemPrompt = '') {
  ...
  const timeout = parseInt(process.env.AURORA_AI_TIMEOUT || "45000");
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      throw new Error(`Timeout: La petición a Groq excedió los ${timeout}ms`);
    }
    throw error;
  }
}
...
async function callKimi(prompt, systemPrompt = '') {
  ...
  try {
    ...
    clearTimeout(id);
    if (error.name === 'AbortError') {
      throw new Error(`Timeout: La petición a NVIDIA/Kimi excedió los ${timeout}ms`);
    }
    throw error;
  }
}
...
async function callOpenRouter(prompt, modelType = 'code', systemPrompt = '') {
  ...
  try {
    ...
    clearTimeout(id);
    if (error.name === 'AbortError') {
      throw new Error(`Timeout: La petición a OpenRouter excedió los ${timeout}ms`);
    }
    throw error;
  }
}
...
function classifyTask(prompt) {
  ...
  const defaultProvider = providerSelector.select('groq', ['openrouter', 'kimi'], availability, 0.2);
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
        name: 'ai_chat',
        description: 'Chat con AI multi-provider (Groq, Kimi, OpenRouter). Selecciona automáticamente el mejor modelo según la tarea.',
        inputSchema: {
          type: 'object',
          properties: {
            prompt: {
            ...
            provider: {
              type: 'string',
              enum: ['auto', 'groq', 'kimi', 'openrouter'],
              description: 'Proveedor (auto = selección automática)'
            },
            model: {
              type: 'string',
      ...
      case 'ai_chat': {
        ...
        if (provider === 'auto') {
          ...
          switch (classification.provider) {
            case 'groq':
              result = await aiThrottler.enqueue('groq', () => callGroq(prompt, classification.model, systemPrompt));
              break;
            case 'kimi':
              result = await aiThrottler.enqueue('kimi', () => callKimi(prompt, systemPrompt));
              break;
            case 'openrouter':
              result = await aiThrottler.enqueue('openrouter', () => callOpenRouter(prompt, classification.model, systemPrompt));
              break;
            default:
          ...
          switch (provider) {
            case 'groq':
              result = await aiThrottler.enqueue('groq', () => callGroq(prompt, model, systemPrompt));
              break;
            case 'kimi':
              result = await aiThrottler.enqueue('kimi', () => callKimi(prompt, systemPrompt));
              break;
            case 'openrouter':
              result = await aiThrottler.enqueue('openrouter', () => callOpenRouter(prompt, model, systemPrompt));
              break;
            default:
...
3. Código reescrito con las mejoras aplicadas`;
        ...
        const result = await aiThrottler.enqueue('kimi', () => callKimi(prompt, systemPrompt));
...
Devuelve solo el código, sin explicaciones.`;
        ...
        const result = await aiThrottler.enqueue('groq', () => callGroq(prompt, fast ? 'fast' : 'quality', systemPrompt));
