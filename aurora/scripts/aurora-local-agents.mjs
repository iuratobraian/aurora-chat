...
export function getLocalAgentStatus() {
  const ollamaPath = safeExec('powershell -NoProfile -Command "(Get-Command ollama).Source"');
  const opencodePath = safeExec('powershell -NoProfile -Command "(Get-Command opencode).Source"');
  const codexPath = safeExec('powershell -NoProfile -Command "(Get-Command codex).Source"');
