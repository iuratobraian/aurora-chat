...
export const getChannels = query({
  ...
  handler: async (ctx, args) => {
    ...
    const enriched = await Promise.all(channels.map(async (c) => {
      ...
      let otherUser = null;
      if (c.type === 'direct' && args.userId) {
        const otherUserId = c.participants?.find(p => p !== args.userId);
        if (otherUserId) {
          otherUser = await ctx.db.get(otherUserId as any);
        }
      }
...
export const createChannel = mutation({
  args: {
    ...
    initialParticipants: v.optional(v.array(v.string())),
  },
  ...
  handler: async (ctx, args) => {
    ...
    const channelId = await ctx.db.insert("chatChannels", {
      ...
      slug,
      type: "private",
      participants: [args.createdBy, ...(args.initialParticipants || [])],
...
export const deleteMessage = mutation({
  args: { messageId: v.id("chat"), userId: v.string() },
  handler: async (ctx, args) => {
    const msg = await ctx.db.get(args.messageId);
    if (!msg) throw new Error("Mensaje no encontrado");
    if (msg.userId !== args.userId) throw new Error("Sin permisos");
    await ctx.db.patch(args.messageId, { isDeleted: true });
  },
});

export const editMessage = mutation({
  args: { messageId: v.id("chat"), userId: v.string(), newTexto: v.string() },
  handler: async (ctx, args) => {
    const msg = await ctx.db.get(args.messageId);
    if (!msg) throw new Error("Mensaje no encontrado");
    if (msg.userId !== args.userId) throw new Error("Sin permisos");
