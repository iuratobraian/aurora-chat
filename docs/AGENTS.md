...
## 🚀 Notion Master Protocol (OBLIGATORIO)

**Notion es la única fuente de verdad.** TASK_BOARD.md es solo un espejo local.

### 1. OBJETIVO DEL SISTEMA
Operar de forma autónoma, organizando, ejecutando y manteniendo tareas sin generar caos, duplicaciones ni errores estructurales.

### 2. ESTRUCTURA DE DATOS
* **DATABASE: IDEAS** (Ideas de alto nivel, una por funcionalidad/mejora)
* **DATABASE: TASKS** (Tareas técnicas ejecutables vinculadas a una "Source Idea")
* **FLUJO:** IDEA → TASKS → READY → IN PROGRESS → REVIEW → DONE

### 3. PROTOCOLO DE ARRANQUE (MANDATORIO)
```bash
# 1. ACTUALIZAR CÓDIGO (Obligatorio antes de empezar)
...
### 3.1 PROTOCOLO DE SINCRONIZACIÓN (OBLIGATORIO)
...
│         ↓                                                        │
│  2. node scripts/aurora-notion-sync.mjs  ← Lee Notion          │
│         ↓                                                        │
│  3. [PRIMER AGENTE] Actualizar TASK_BOARD.md con tareas        │
│         ↓                                                        │
│  4. git add + commit + push  ← Sincroniza para otros agentes   │
│         ↓                                                        │
│  5. Otros agentes leen TASK_BOARD.md local (ya actualizado)    │
└─────────────────────────────────────────────────────────────────┘
```

**El primer agente que inicia sesión DEBE:**
1. Ejecutar `node scripts/aurora-notion-sync.mjs`
2. Actualizar `TASK_BOARD.md` con las tareas de Notion
3. Hacer `git push` para que otros agentes vean los cambios

**Otros agentes** pueden leer `TASK_BOARD.md` directamente (ya sincronizado).

### 4. REGLAS DE OPERACIÓN PARA AGENTES
- **CÓMO LEER:** Name → Domain → Dependencies → Definition of Done (DoD).
- **SELECCIÓN:** Solo tomar tareas con `Status = Ready` y `Blocked = false`.
- **ANTI-DUPLICADOS:** Antes de trabajar, cambiar `Ready → In Progress` para bloquear a otros.
- **EJECUCIÓN:** Implementar SOLO lo pedido. No improvisar ni mezclar dominios.
- **FINALIZACIÓN:** Validar DoD y cambiar `In Progress → Review`.
- **PROHIBIDO:** Trabajar tareas en Backlog/Blocked o duplicar lógica.

### 5. ARQUITECTURA POR DOMINIOS
Dominios permitidos: `Auth`, `Users`, `Social`, `Communities`, `Payments`, `Signals`, `Realtime`, `Notifications`, `Core`.

### 6. GESTIÓN DE IDEAS Y TAREAS
- **Nuevas Ideas:** Crear en IDEAS con estado "Procesando".
- **Descomposición:** Dividir en tareas pequeñas (1 responsabilidad por tarea) con DoD claro y Domain asignado.
- **Ready vs Blocked:** Una tarea es `Ready` si no tiene dependencias activas.

## 🧠 Aurora AI Multi-Agent System

Este proyecto integra **Aurora AI** - un sistema multi-agente inteligente con 7 providers y 8 subagentes especializados.
...
| Provider | Modelo | Velocidad | Costo | Mejor Para |
|----------|--------|-----------|-------|------------|
| **Groq** | Llama 3.3 70B | ⚡ 700ms | $0.0004/1k | Código, debugging |
| **Kimi K2** | kimi-k2-instruct | 🧠 3-5s | $0.0005/1k | Code review, arquitectura |
| **GLM-4** | chatglm3-6b | 💎 2-3s | **GRATIS** | Explicaciones, tutoriales |
| **Claude 3.5** | claude-3-5-sonnet | ⭐ 3-5s | $0.003/1k | Premium, producción |
| **OpenRouter** | Qwen2.5 Coder | 💰 2-3s | $0.0002/1k | Backup económico |
| **Ollama** | qwen2.5-coder | 🔒 10-30s | GRATIS | Offline |
...
### Routing Inteligente
El sistema selecciona automáticamente el mejor provider según la tarea:
- **Code review, seguridad** → Kimi K2 + subagents
- **Generar código** → Groq 70B
- **Explicaciones** → GLM-4 (¡GRATIS!)
- **Premium** → Claude 3.5
...
Configurado en `.mcp.json` para usar desde Claude Code:
```
/aurora-ai ai_chat
  prompt: "Genera hook useFetch"
  provider: auto
  model: quality
```
...
1) BEFORE STARTING: ALWAYS run `git pull` to ensure local files are in sync with remote changes.
2) Read `.agent/skills/README.md`, `.agent/skills/mandatory-startup-readiness/SKILL.md`, `.agent/skills/mandatory-startup-readiness/references/critical-failures.md` and `AGENT_TASK_DIVISION.md`.
3) Choose a task from Notion "READY TASKS" view (or `.agent/workspace/coordination/TASK_BOARD.md` if Notion is not available). Choice criteria: Status = Ready, Blocked = false. Do NOT pick at random.
4) Read: Name, Domain, Dependencies, and Definition of Done before starting.
5) Add a claim (status `claimed` or `In Progress`, your agent name and date).
6) Add `CURRENT_FOCUS.md` entry with files to touch and files forbidden.
7) Make changes only within declared scope. Implement ONLY what is defined; no extra logic.
8) Run `npm run lint` and the relevant tests locally before committing.
9) Update `AGENT_LOG.md` with a short, verifiable entry (date, TASK-ID, files changed, validation, remaining risk).
10) Validate Definition of Done for each task.
11) Mark task as `done` in TASK_BOARD.md and update Notion Status: `In Progress` -> `Review`.
12) **Auto-sync to Notion**: Run `node scripts/notion-auto-sync.mjs` to push status updates.
13) **AFTER FINISHING**: ALWAYS run `git push` to synchronize changes with the remote repository. Ensure the system is intact.
14) If handing off, use `.agent/workspace/coordination/HANDOFFS.md` and `HANDOFF_TEMPLATE.md`.
15) **IF NO TASKS ARE READY**: Do nothing and wait for new tasks. Do not invent work.
...
## 🚀 Git Operations - Authorized Users Only (MANDATORY)
**Solo se permite realizar `git commit` y `git push` con los siguientes usuarios:**
1.  **iuratobraian**
2.  **iuratobraian-4827**

Cualquier commit con otro usuario debe ser rechazado o corregido antes de push. El script `scripts/deploy.ps1` valida esto automáticamente.
...
- **AUTONOMÍA TOTAL (Loop Infinito)**: El comando `inicio` activa un ciclo de trabajo ininterrumpido. Al finalizar un lote de tareas, el agente DEBE seleccionar automáticamente el siguiente lote de tareas `pending` del `TASK_BOARD.md` (y confirmar en Notion "READY TASKS") sin preguntar, hasta que el tablero esté vacío. **PROHIBIDO** detenerse o pedir instrucciones si hay trabajo pendiente.
- **FLUJO MANDATORIO**: Al iniciar el comando `inicio`, se DEBE ejecutar `git pull`. Al finalizar cada tarea exitosa, se DEBE ejecutar `git push`. No omitir estos pasos bajo ninguna circunstancia.
...
- **NOTION FIRST**: El sistema de tareas en Notion es la fuente de verdad. Los agentes DEBEN operar de forma autónoma respetando el sistema sin intervención humana. Si faltan credenciales o acceso, reportar inmediatamente.
...
Integración con Qwen para code review y consultas pre-implementación.

### Script Principal
- `scripts/aurora-qwen-agent.ts` - Agente consultor de código

### Uso como módulo (para agentes)
```javascript
import { askQwen, askQwenCodeReview, askQwenPreImpl } from "./scripts/aurora-qwen-agent.ts";

// Code review antes de implementar
const review = await askQwenCodeReview(["convex/posts.ts"], "Seguridad");

// Consulta pre-implementación
const result = await askQwenPreImpl(
  "Agregar paginación al feed",
  ["convex/feed.ts", "src/hooks/useFeed.ts"],
  ["No usar collect() sin límite"]
);
```
...
QWEN_MODEL=qwen-turbo  # o qwen-plus, qwen-max
```

### FLUJO OBLIGATORIO
```
┌─────────────────────────────────────────────────────────┐
│  ANTES DE IMPLEMENTAR (TAREAS NUEVAS)                    │
│                                                         │
│  1. askQwenPreImpl() → Revisar enfoque                  │
│  2. Implementar código                                   │
│  3. askQwenCodeReview() → Detectar issues              │
│  4. Corregir issues                                     │
│  5. Commit                                              │
└─────────────────────────────────────────────────────────┘
```
...
- ✅ Pre-implementación (detectar problemas de arquitectura)
- ✅ Refactoring suggestions
- ✅ Query optimization

### Para qué NO usar Qwen:
- ❌ Modificar archivos directamente
- ❌ Decisiones de negocio (para eso es Kimi)
