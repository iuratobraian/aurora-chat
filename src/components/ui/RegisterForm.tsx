...
interface RegisterFormProps {
  onSubmit: (data: { name: string; email: string; password: string; referralCode?: string }) => void;
  onLogin?: () => void;
  onGoogleSignIn?: () => void;
  loading?: boolean;
  error?: string;
...
export const RegisterForm: React.FC<RegisterFormProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit({ name, email, password, referralCode: referralCode || undefined });
  };
  ...
  return (
    <div className={`w-full max-w-md ${className}`}>
      <div className="text-center mb-8">
        ...
        <p className="text-gray-400 mt-2">Crea tu cuenta en TradeShare</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="ABC123"
            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all duration-300 uppercase"
          />
        </div>
          ...
          <>
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-[#0f1115] text-gray-400">o regístrate con</span>
              </div>
            </div>
