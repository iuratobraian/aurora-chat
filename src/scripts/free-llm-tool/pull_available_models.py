...
# Global clients
mistral_client = None
if Mistral and os.environ.get("MISTRAL_API_KEY"):
    try:
...
def fetch_cloudflare_models(logger):
    ...
    r = requests.get(
        f"https://api.cloudflare.com/client/v4/accounts/{os.environ.get('CLOUDFLARE_ACCOUNT_ID')}/ai/models/search?search=Text+Generation",
        headers={
...
def fetch_hyperbolic_models(logger):
    if not os.environ.get("HYPERBOLIC_API_KEY"):
        logger.warning("HYPERBOLIC_API_KEY not found, skipping Hyperbolic.")
        return []
    ...
    try:
        ...
        logger.error(f"Error fetching Hyperbolic models: {e}")
        return []
...
def fetch_gemini_limits(logger):
    if not cloudquotas_v1:
        logger.warning("google-cloud-quotas not installed, skipping Gemini limits fetch.")
        return {}
    logger.info("Fetching Gemini limits...")
    try:
        client = cloudquotas_v1.CloudQuotasClient()
        request = cloudquotas_v1.ListQuotaInfosRequest(
            parent=f"projects/{os.environ.get('GCP_PROJECT_ID')}/locations/global/services/generativelanguage.googleapis.com"
        )
...
def fetch_lambda_models(logger):
    ...
    r = requests.get(
        ...
        headers={
            "Authorization": f"Bearer {os.environ.get('LAMBDA_API_KEY')}",
        },
    )
    r.raise_for_status()
    models = r.json()["data"]
...
def fetch_scaleway_models(logger):
    ...
    r = requests.get(
        ...
        headers={"Authorization": f"Bearer {os.environ.get('SCALEWAY_API_KEY')}"},
    )
    r.raise_for_status()
    models = r.json()["data"]
